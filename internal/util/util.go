...
// the randondb-postgres-gis-ha container
...
// randondb-postgres-gis-ha container with the corresponding tag, it returns
...
func GetStandardImageTag(imageName, imageTag string) string {
	if imageName == "randondb-postgres-gis-ha" && strings.Count(imageTag, "-") > 2 {
...
// who worked on the patch to add this, prior to being at Radondb Data
