...
func (c *Controller) processNextItem() bool {
	...
	// get the pgcluster
	cluster, err := c.Client.QingcloudV1().Pgclusters(keyNamespace).Get(ctx, keyResourceName, metav1.GetOptions{})
	if err != nil {
	...
	if err == nil {
		_, err = c.Client.QingcloudV1().Pgclusters(keyNamespace).
...
func (c *Controller) onDelete(obj interface{}) {
	...
	if err := c.Client.QingcloudV1().Pgtasks(cluster.Namespace).Delete(
...
func updateLabelsForConfigMaps(c *Controller, cluster *crv1.Pgcluster, labels map[string]string, labelsToRemove []string) error {
	...
	options := metav1.ListOptions{
		LabelSelector: fields.AndSelectors(
			fields.OneTermEqualSelector(config.LABEL_PG_CLUSTER, cluster.Name),
			fields.OneTermEqualSelector(config.LABEL_VENDOR, config.LABEL_QINGCLOUD),
		).String(),
	}
...
func updateServices(clientset kubeapi.Interface, cluster *crv1.Pgcluster) {
	...
	replicas, err := clientset.QingcloudV1().Pgreplicas(cluster.Namespace).List(ctx, options)
