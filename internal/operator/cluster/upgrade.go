...
import (
	...
	pgoconfig "github.com/qingcloud/postgres-operator/internal/operator/config"
...
const (
	postgresImage      = "qingcloud-postgres"
	postgresHAImage    = "qingcloud-postgres-ha"
	postgresGISImage   = "qingcloud-postgres-gis"
	postgresGISHAImage = "qingcloud-postgres-gis-ha"
)
...
ForceCommand NSS_WRAPPER_SUBDIR=ssh . /opt/qingcloud/bin/nss_wrapper_env.sh && $SSH_ORIGINAL_COMMAND`
...
func AddUpgrade(clientset kubeapi.Interface, upgrade *crv1.Pgtask, namespace string) {
	...
	pgcluster, err := clientset.QingcloudV1().Pgclusters(namespace).Get(ctx, upgradeTargetClusterName, metav1.GetOptions{})
	if err != nil {
	...
	// update the unix socket directories parameter so it no longer include /qingcloudadm and
	// set any path references to the /opt/qingcloud... paths
	...
	_, err = clientset.QingcloudV1().Pgclusters(namespace).Create(ctx, pgcluster, metav1.CreateOptions{})
	if err != nil {
...
func handleReplicas(clientset kubeapi.Interface, clusterName, currentPrimaryPVC, namespace string) string {
	...
	numReps := 0
	replicaList, err := clientset.QingcloudV1().Pgreplicas(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
	...
	for index := range replicaList.Items {
		if replicaList.Items[index].Spec.ClusterName == clusterName {
			...
			_ = clientset.QingcloudV1().Pgreplicas(namespace).Delete(ctx, replicaList.Items[index].Name, metav1.DeleteOptions{})
...
func deleteBeforeUpgrade(clientset kubeapi.Interface, pgcluster *crv1.Pgcluster, currentPrimary, namespace string) error {
	...
	if _, err := clientset.QingcloudV1().Pgclusters(namespace).Update(ctx,
	...
	_ = clientset.QingcloudV1().Pgclusters(namespace).Delete(ctx, pgcluster.Name, metav1.DeleteOptions{})
...
func deleteNonupgradePgtasks(clientset pgo.Interface, selector, namespace string) error {
	...
	for _, v := range taskList.Items {
		...
		if v.ObjectMeta.Name != v.Name+"-"+config.LABEL_UPGRADE {
			err = clientset.QingcloudV1().Pgtasks(namespace).Delete(ctx, v.Name, metav1.DeleteOptions{})
			if err != nil {
				return err
			}
		}
...
func preparePgclusterForUpgrade(pgcluster *crv1.Pgcluster, parameters map[string]string, oldpgoversion, currentPrimary string) {
	...
	// next, capture the existing Qingcloud Postgres Exporter configuration settings (previous to version
	// 4.5.0 referred to as Qingcloud Collect), if they exist, and store them in the current labels
	// 4.6.0 added this value to the spec as "Exporter", so the next step ensure
	// that the value is migrated over
	if value, ok := pgcluster.ObjectMeta.Labels["qingcloud_collect"]; ok {
		pgcluster.ObjectMeta.Labels[config.LABEL_EXPORTER] = value
	}
	delete(pgcluster.ObjectMeta.Labels, "qingcloud_collect")

	// Note that this is the *user labels*, the above is in the metadata labels
	if value, ok := pgcluster.Spec.UserLabels["qingcloud_collect"]; ok {
		pgcluster.Spec.UserLabels[config.LABEL_EXPORTER] = value
	}
	delete(pgcluster.Spec.UserLabels, "qingcloud_collect")
	...
	if ok, _ := strconv.ParseBool(pgcluster.ObjectMeta.GetLabels()["qingcloud-pgbadger"]); ok {
		pgcluster.Spec.PGBadger = true
	}
	delete(pgcluster.ObjectMeta.Labels, "qingcloud-pgbadger")
...
func createClusterRecreateWorkflowTask(clientset pgo.Interface, clusterName, ns, pgouser string) (string, error) {
	...
	_, err = clientset.QingcloudV1().Pgtasks(ns).Create(ctx, newInstance, metav1.CreateOptions{})
	if err != nil {
		log.Error(err)
		return "", err
	}
...
func updateUpgradeWorkflow(clientset pgo.Interface, namespace, workflowID, status string) error {
	...
	if _, err := clientset.QingcloudV1().Pgtasks(namespace).Update(ctx, &task, metav1.UpdateOptions{}); err != nil {
...
func updateClusterConfig(clientset kubeapi.Interface, pgcluster *crv1.Pgcluster, namespace string) error {
	...
	// as of version 4.6, the /qingcloudadm directory no longer exists (previously set as a unix socket directory)
	// and the /opt/cpm... directories are now set under /opt/qingcloud
	if dcsConf.PostgreSQL != nil && dcsConf.PostgreSQL.Parameters != nil {
		...
		case operator.IsLocalAndS3Storage(pgcluster):
			dcsConf.PostgreSQL.Parameters["archive_command"] = `source /opt/qingcloud/bin/postgres-ha/pgbackrest/pgbackrest-archive-push-local-s3.sh %p`
		case operator.IsLocalAndGCSStorage(pgcluster):
			dcsConf.PostgreSQL.Parameters["archive_command"] = `source /opt/qingcloud/bin/postgres-ha/pgbackrest/pgbackrest-archive-push-local-gcs.sh %p`
		default:
			dcsConf.PostgreSQL.Parameters["archive_command"] = `source /opt/qingcloud/bin/postgres-ha/pgbackrest/pgbackrest-set-env.sh && pgbackrest archive-push "%p"`
		}

		dcsConf.PostgreSQL.RecoveryConf["restore_command"] = `source /opt/qingcloud/bin/postgres-ha/pgbackrest/pgbackrest-set-env.sh && pgbackrest archive-get %f "%p"`
	}

	if localDBConf.PostgreSQL.Callbacks != nil {
		localDBConf.PostgreSQL.Callbacks.OnRoleChange = "/opt/qingcloud/bin/postgres-ha/callbacks/pgha-on-role-change.sh"
	}
	if localDBConf.PostgreSQL.PGBackRest != nil {
		localDBConf.PostgreSQL.PGBackRest.Command = "/opt/qingcloud/bin/postgres-ha/pgbackrest/pgbackrest-create-replica.sh replica"
	}
	if localDBConf.PostgreSQL.PGBackRestStandby != nil {
		localDBConf.PostgreSQL.PGBackRestStandby.Command = "/opt/qingcloud/bin/postgres-ha/pgbackrest/pgbackrest-create-replica.sh standby"
	}
